...
class SpotifyPlaylistTools(Toolkit):
    def __init__(self, access_token: str):
        ...
        self.register(self.get_top_tracks_by_genre)
    ...
    def create_playlist_by_uris(
        self,
        songs_uris: List[str],
        playlist_name="made by NTTLuke (with Agno)",
    ) -> str:
        """
    ...
    def get_friday_new_releases(self, limit: int = 10, country: str = "IT") -> str:
        """
    ...
    def get_top_tracks_by_genre(self, genre: str, time_range: str = "medium_term", limit: int = 10) -> List[Dict[str, str]]:
        """
        Returns the user's top tracks filtered by a given genre.

        Args:
            genre: The genre to filter tracks by (case-insensitive, partial match allowed).
            time_range: 'long_term' (~1yr), 'medium_term' (~6mo), or 'short_term' (~4wk).
            limit: The number of top tracks to retrieve (max 50).

        Returns:
            A list of dicts with 'title' and 'artist' for tracks matching the genre.
        """
        logger.info(f"Getting top tracks for time range: {time_range} and filtering by genre: {genre}")
        url = f"{self.base_url}/me/top/tracks"
        params = {"time_range": time_range, "limit": min(limit, 50)}
        result = self._api_request("GET", url, params=params)

        if isinstance(result, str):
            logger.warning(f"Failed to fetch top tracks: {result}")
            return []

        if "items" not in result:
            logger.warning("No 'items' in top tracks response.")
            return []
        ...
        for track in tracks:
            for artist in track.get("artists", []):
                if artist.get("id"):
                    artist_ids.add(artist["id"])
                    artist_id_to_name[artist["id"]] = artist.get("name", "Unknown")

        # Fetch genres for all unique artists (in batches of 50)
        genres_by_artist = {}
        artist_ids_list = list(artist_ids)
        for i in range(0, len(artist_ids_list), 50):
            batch_ids = artist_ids_list[i : i + 50]
        ...
        for track in tracks:
            ...
            for artist in track_artists:
                artist_id = artist.get("id")
                artist_genres = genres_by_artist.get(artist_id, [])
                if any(genre_lower in g.lower() for g in artist_genres):
                    match = True
                    break
            if match:
                filtered_tracks.append({
                    "title": track_title,
                    "artist": ", ".join([a.get("name", "Unknown") for a in track_artists])
                })
